...
try {
  ...
  $statement = $pdo->prepare("SELECT * FROM vacancy_participants WHERE (status=0 AND role=1 AND account_number ILIKE '$account_number')");
  $statement->execute();
  $participantsList = $statement->fetchAll(PDO::FETCH_ASSOC);
  $myRequests = [];
  for ($i = 0; $i < count($participantsList); $i++) {
    $id = $participantsList[$i]["id_request"];
    $statement = $pdo->prepare("SELECT * FROM vacancy_request WHERE (status=3 AND id=$id)");
    $statement->execute();
