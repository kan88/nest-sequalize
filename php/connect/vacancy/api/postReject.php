...
try {
  ...
  if ($role != null) {
    for ($i = 0; $i < count($role); $i++) {
      if ($i == 0) {
        $statement = $pdo->prepare('UPDATE vacancy_participants SET reason_reject=:reason_reject, role=:role WHERE id=:id');
        $statement->execute([
          'id' => $id,
          'role' => $role[$i],
        ...
        $statement->execute([
          ...
          'is_view' => $is_view[$i - 1],
          'is_interview' => $is_interview[$i - 1],
          'is_approve' => $is_approve[$i - 1],
