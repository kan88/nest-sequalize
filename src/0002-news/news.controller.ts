...
import { UpdateLikeDto } from 'src/0002-likes/dto/update-like.dto';
import { LikesService } from 'src/0002-likes/likes.service';
import { Like } from 'src/0002-likes/likes.model';
import { PhotosService } from 'src/0002-photos/photo.service';
...
export class NewsController {
  ...
  async update(
    ...
    if (data[1] === false) {
      dto.news_like === '0'
        ? (response = await this.newsService.decrementLikes(id))
        : (response = await this.newsService.incrementLikes(id));
    }
    return response;
  }
  ...
  async removePoto(@Param('id_image', ParseIntPipe) id_image: number) {
